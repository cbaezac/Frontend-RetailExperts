(function () {
  ...
  var delLabel = document.getElementById('delLabel');
  ...
  deleteBtn = clearOldHandlers(deleteBtn);
  ...
  function updateSelection() {
    ...
    if (!count) {
      ...
      delLabel.textContent = 'Eliminar (' + count + ')';
    }
  ...
  function loadPhotos(options) {
    ...
    updatePager();
    return window.RetailAPI.requestJson('/web/admin/galeria/fotos' + window.RetailAPI.buildQuery(collectParams()))
  ...
  deleteBtn.addEventListener('click', function () {
    var selectedPhotos = Object.keys(selected).map(function (id) { return selected[id]; });
    if (selectedPhotos.length === 0) return;
    var message = selectedPhotos.length === 1
      ? 'Cuidado, ¿estás seguro de querer eliminar esta foto?'
      : 'Cuidado, ¿estás seguro de querer eliminar estas fotos?';
    if (!window.confirm(message)) return;

    deleteBtn.disabled = true;
    delLabel.textContent = 'Eliminando...';
    window.RetailAPI.requestJson('/web/admin/galeria/fotos/soft-delete', {
      method: 'PATCH',
      body: JSON.stringify({
        ids: selectedPhotos.map(function (photo) { return Number(photo.id_foto || photo.id); }),
        reason: 'Eliminada desde panel admin'
      })
    }).then(function () {
      selected = {};
      loadPhotos();
    }).catch(function () {
      window.alert('No se pudieron eliminar las fotografías seleccionadas.');
    }).finally(function () {
  ...
  window.RetailAPI.requestJson('/web/admin/galeria/filtros')
    .then(function (payload) {
